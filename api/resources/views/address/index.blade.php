                ...
                <div class="p-6 dark:text-white bg-white dark:bg-gray-800 border-b border-gray-200">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        @foreach ($addresses as $address)
                            <tr {{ $default_address_id == $address->id ? 'class=default_address_id' : '' }}>
                                <td>
                                    ...
                                    <form method="POST"
                                            action="{{ route('address.setDefault', ['id' => $address->id]) }}">
                                            @csrf
                                            @method('PUT')
                                ...
                                <td class="acoes_evt">
                                    <a class="text-gray-500 dark:text-gray-400"
                                        href="{{ route('address.edit', ['address' => $address->id]) }}">Editar</a>
                                    @if (!($default_address_id == $address->id))
                                        <form method="POST"
